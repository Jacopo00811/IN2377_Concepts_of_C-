...
public:
     ...
     */
    //constexpr bool contains(const K &key) const { std::find_if(values.begin(), values.end(), [&key](const auto& pair) { return pair.first == key; }) != values.end(); }
    constexpr bool contains(const K &key) const { return find(key) != values.end(); }
    
    /**
     * Get a key's value
     */
    //constexpr const V &get(const K &key) const { 
    //    auto it = std::find_if(velues.begin(), values.end(), [&key](const auto & pair){ return pair.first; });
    //    return it->second; 
    //}
    constexpr const V &get(const K &key) const {
        auto it = find(key);
     ...
     */
    constexpr const V &operator [](const K &key) const { return get(key); }
...
constexpr auto create_cexpr_map(Entries&&... entry) { return CexprMap<K, V, sizeof...(Entries)>{std::forward<Entries>(entry)...}; }
...
CexprMap(Entry, Rest&&...) -> CexprMap<std::decay_t<typename Entry::first_type>, std::decay_t<typename Entry::second_type>, sizeof...(Rest) + 1>;
